...
export default function OrganizationDetailPage() {
  ...
  const existingUserIds = new Set(members.filter(m => !m.dateLeft).map(m => m.userId));
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={removeTarget !== null} onOpenChange={() => { setRemoveTarget(null); setRemoveError(''); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Remove Member</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            Are you sure you want to remove this member? Their record will be kept with today as the leave date.
          </p>
          {removeError && <p className="text-sm text-destructive">{removeError}</p>}
          <DialogFooter>
            <Button variant="outline" onClick={() => { setRemoveTarget(null); setRemoveError(''); }}>Cancel</Button>
            <Button
              variant="destructive"
              disabled={removeMutation.isPending}
              onClick={() => removeTarget !== null && removeMutation.mutate(removeTarget)}
            >
